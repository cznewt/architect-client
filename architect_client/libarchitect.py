...
class ArchitectClient(object):
    ...
    def _req_post_json(self, path, data):
        ...
        headers = {"Accept": "application/json", "Content-Type": "application/json"}
        # self._ssl_verify = self.ignore_ssl_errors
        data = data.update({'access_token': self.token})
    ...
    def get_data(self, resource=None):
        if resource is None:
            path = "/inventory/v1/inventory-data/{}/?token={}".format(
                self.inventory, self.token
            )
        else:
            path = "/inventory/v1/minion-data/{}/?token={}".format(
                resource, self.token
            )
